...
public class CollectionCommonUtils {
    ...
    public static <T,R extends Comparable<R>> ListCompareResult<T> compareList(List<T> oldList, List<T> newList,Function<T,R> comparableConvertor,boolean allowRepeatCompareKey){
        Map<R, T> oldMap = convertCollection2Map(oldList, comparableConvertor, Function.identity(), allowRepeatCompareKey);
        Map<R, T> newMap = convertCollection2Map(newList, comparableConvertor, Function.identity(), allowRepeatCompareKey);
        List<R> oldCompareList = oldList.stream().map(comparableConvertor).collect(Collectors.toList());
        List<R> newCompareList = newList.stream().map(comparableConvertor).collect(Collectors.toList());
        ListCompareResult<R> compareResult = compareList(oldCompareList, newCompareList);
        ListCompareResult<T> realCompareResult = new ListCompareResult<>();
        realCompareResult.setAddList(compareResult.getAddList().stream().map(newMap::get).collect(Collectors.toList()));
        realCompareResult.setRemoveList(compareResult.getRemoveList().stream().map(oldMap::get).collect(Collectors.toList()));
        realCompareResult.setNoChangeList(compareResult.getNoChangeList().stream().map(newMap::get).collect(Collectors.toList()));
        return realCompareResult;
    }
     ...
     *
     * @param collection 集合
     * @param newCollection       新元素
     * @param <T>        T
     */
    public static <T> void addNoNullItem(Collection<T> collection, Collection<T> newCollection) {
        if (nonNull(newCollection)) {
            newCollection.stream().filter(Objects::nonNull).collect(Collectors.toList());
